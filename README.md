...
本ツールはポケモンSVのシングル対戦（主にランクマッチ）におけるダメージ計算やパーティ管理、画面認識機能や対戦記録保存・分析機能を半自動で行うツールである
...
- 暇ツール（暇士様作成）
  - ダメージ計算ロジックやポケモンや技のDB情報や画像、ボタンやフレームなどの各モジュールを流用させていただきました
- ぱにぱにツール（ぱにぱに様作成）
  - ポケモン画像や履歴、分析画面のベースを流用させていただきました

特に許可を得ているわけではないので、個人の利用に留めます
...
※ 2025/07/24 以降、ダブルバトル対応に伴って、DBをアップデートする必要があります。それまで利用されていた方は開発者から更新用バッチを受け取ってください
...
- 努力値・ランクはカーソルか手動入力で設定
  - 努力値は右クリックで0と252を切り替え
  - ランクは1ずつ増減
  - それぞれのボタンを押すと一括削除可能
- 個体値は項目のボタンを押すことで0－31を入れ替えられます
